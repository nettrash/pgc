//! Sequences (`pg_sequence`) — `CREATE SEQUENCE`.
//!
//! Sequences owned by a `serial` or identity column are *not* independent objects:
//! they are created and dropped with their column, and emitting them separately
//! would produce a migration that fails to apply. The comparer detects them and
//! routes the rest through its `sequence_post_script` buffer.
...
pub struct Sequence {
    /// Schema where the sequence is defined
    pub schema: String,
    ...
    /// Schema of the owning table/column
    pub owned_by_schema: Option<String>,
    /// Owning table name
    pub owned_by_table: Option<String>,
    /// Owning column name
    pub owned_by_column: Option<String>,
    /// Whether the sequence is an identity sequence
    pub is_identity: bool,
    /// Whether the sequence is unlogged (relpersistence = 'u')
    #[serde(default)]
    pub is_unlogged: bool,
    /// Optional sequence comment
    #[serde(default)]
    pub comment: Option<String>,
    /// Hash of the sequence
    pub hash: Option<String>,
    /// ACL (grant) entries for this sequence
