...
impl TableConstraint {
    ...
    pub(super) fn strip_not_null_prefix(definition: &str) -> Option<&str> {
    ...
    pub(super) fn parse_pg_identifier(identifier: &str) -> Option<String> {
    ...
    pub(super) fn normalize_identifier_for_name(identifier: &str) -> String {
    ...
    pub fn auto_not_null_column(&self, table_raw: &str) -> Option<String> {
        ...
        // When `{table}_{col}_not_null` (optionally with a numeric collision
        ...
        // when the recorded name would have required truncation — that means
        // either the un-suffixed base alone is too long, OR appending the
        // collision suffix forces re-clipping (e.g. base.len() == 63 and PG
        // appends "1", producing base[..62] + "1"). Note: PG truncates by
        // bytes, so compare byte prefixes (identifiers here are typically
        // ASCII, but `starts_with` on bytes is correct either way).
        const PG_NAMEDATALEN_MAX: usize = 63;
        if base.len() + suffix.len() > PG_NAMEDATALEN_MAX && !head.is_empty() {
