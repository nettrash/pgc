...
impl TableConstraint {
    ...
    /// numeric suffix that PG appends to resolve collisions across tables),
    /// returns the lowercased column name. Otherwise returns `None`.
    ///
    ...
    pub fn auto_not_null_column(&self, table_raw: &str) -> Option<String> {
        ...
        let def_lower = self.definition.as_deref()?.to_lowercase();
        let col = def_lower
            .strip_prefix("not null ")?
            .trim()
            .trim_matches('"')
            .to_string();
        let base = format!("{}_{}_not_null", table_raw, col).to_lowercase();
        let cname = self.name.trim_matches('"').to_lowercase();
        if cname == base {
            return Some(col);
        }
        let suffix = cname.strip_prefix(&base)?;
