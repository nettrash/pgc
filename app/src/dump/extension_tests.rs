...
fn test_extension_hash_different_for_different_name() {
    let extension1 = Extension::new("ext1".to_string(), "1.0".to_string(), "public".to_string());

    let extension2 = Extension::new("ext2".to_string(), "1.0".to_string(), "public".to_string());
...
fn test_extension_with_very_long_names() {
    ...
    let extension = Extension::new(long_name.clone(), long_version.clone(), long_schema.clone());
...
fn test_extension_deserialize_with_extra_fields() {
    let json = r#"{"name":"postgis","version":"3.3.2","schema":"extensions","extra":"ignored"}"#;
...
fn test_extension_hash_roundtrip_serialization() {
    ...
    let restored: Extension = serde_json::from_str(&json).expect("deserialization should succeed");
