...
fn create_simple_index() -> TableIndex {
    TableIndex {
        ...
        catalog: None,
        indexdef: "CREATE INDEX idx_orders_date ON app.orders USING btree (created_at)".to_string(),
        is_partition_index: false,
        comment: None,
    }
}
...
fn create_partial_index() -> TableIndex {
    TableIndex {
        ...
        catalog: None,
        indexdef:
            "CREATE INDEX idx_products_active ON public.products (name, price) WHERE active = true"
                .to_string(),
        is_partition_index: false,
        comment: None,
    }
...
fn test_different_index_types() {
    let indexes = vec![
        ...
        TableIndex {
            ...
            catalog: None,
            indexdef: "CREATE INDEX gin_idx ON public.documents USING gin (content)".to_string(),
            is_partition_index: false,
            comment: None,
        },
        TableIndex {
...
fn test_is_partition_index_true() {
    let index = TableIndex {
        ...
        catalog: None,
        indexdef:
            "CREATE INDEX orders_2024_created_at_idx ON public.orders_2024 USING btree (created_at)"
                .to_string(),
